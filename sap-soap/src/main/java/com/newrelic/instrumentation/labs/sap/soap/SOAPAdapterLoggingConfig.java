...
public class SOAPAdapterLoggingConfig {
	...
	public SOAPAdapterLoggingConfig() {
		...
		if(logDirExists) {
			File logFile = new File(logDirectory, SOAPAdapterLogger.LOGFILENAME);
			adapterLog = logFile.getAbsolutePath();
		} else {
			File logFile = new File(newRelicDir, SOAPAdapterLogger.LOGFILENAME);
			adapterLog = logFile.getAbsolutePath();
		}
	}
