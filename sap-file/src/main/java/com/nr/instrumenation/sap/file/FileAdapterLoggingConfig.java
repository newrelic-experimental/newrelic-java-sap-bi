...
public class FileAdapterLoggingConfig {

	private String adapterLog = null;
	private int maxLogFiles = 3;
	private String rolloverSize = "100K";
	private int rolloverMinutes = 60;

	public FileAdapterLoggingConfig() {
		...
		if(logDirExists) {
			File logFile = new File(logDirectory, FileAdapterLogger.LOGFILENAME);
			adapterLog = logFile.getAbsolutePath();
		} else {
			File logFile = new File(newRelicDir, FileAdapterLogger.LOGFILENAME);
			adapterLog = logFile.getAbsolutePath();
		}
	}
