...
public class NRLabsFullHandler extends Handler {

	private static final SimpleDateFormat sdf = new SimpleDateFormat("MMMM dd yyyy HH:mm:ss");
	...
	public NRLabsFullHandler(String filename, int rolltime, long size, int max) throws IOException {
	...
	public void publish(LogRecord record) {
		if(!isLoggable(record)) {
			NewRelic.getAgent().getLogger().log(Level.FINE, "in com.newrelic.instrumentation.labs.sap.adapters.NRLabsFullHandler, The LogRecord {0} is not loggable", record);
			return;
		}

		if(logWriter == null) {
			NewRelic.getAgent().getLogger().log(Level.FINE, "in com.newrelic.instrumentation.labs.sap.adapters.NRLabsFullHandler, Cannot write record because logWriter is null: {0}", record);
			return;
		}
		checkForRoll();
		
		Date timestamp = new Date(record.getMillis());
		String msg = sdf.format(timestamp) + " " + record.getMessage();
	...
	public String toString() {
		return "NRLabsFullHandler [maxFiles=" + maxFiles + ", maxSize=" + maxSize + ", rolloverTime=" + rolloverTime
