...
public class ConfigurationListener extends TimerTask {
	...
	@Override
	public void run() {
		NewRelic.getAgent().getLogger().log(Level.FINE, "Running check on SAP attribute configuration file: {0}",configFile);
		try {
			NewRelic.getAgent().getLogger().log(Level.FINE, "Comparing file last modified: {0} to {1}",configFile.lastModified(), lastModified);
