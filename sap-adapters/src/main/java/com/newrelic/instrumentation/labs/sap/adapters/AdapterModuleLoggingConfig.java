...
public class AdapterModuleLoggingConfig {
	...
	public AdapterModuleLoggingConfig() {
		...
		if(logDirExists) {
			File logFile = new File(logDirectory, AdapterModuleLogger.LOGFILENAME);
			adapterLog = logFile.getAbsolutePath();
		} else {
			File logFile = new File(newRelicDir, AdapterModuleLogger.LOGFILENAME);
			adapterLog = logFile.getAbsolutePath();
		}
	}
