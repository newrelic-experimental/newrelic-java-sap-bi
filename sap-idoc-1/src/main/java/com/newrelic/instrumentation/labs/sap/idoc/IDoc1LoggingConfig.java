...
public class IDoc1LoggingConfig {
	...
	public IDoc1LoggingConfig() {
		...
		if(logDirExists) {
			File logFile = new File(logDirectory, IDoc1Logger.LOGFILENAME);
			adapterLog = logFile.getAbsolutePath();
		} else {
			File logFile = new File(newRelicDir, IDoc1Logger.LOGFILENAME);
			adapterLog = logFile.getAbsolutePath();
		}
	}
