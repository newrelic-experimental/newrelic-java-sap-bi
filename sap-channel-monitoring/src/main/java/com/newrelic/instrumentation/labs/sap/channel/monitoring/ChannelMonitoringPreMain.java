...
public class ChannelMonitoringPreMain {
	...
	public static void init() {
		if(!isSAP()) {
			NewRelic.getAgent().getLogger().log(Level.FINE, "Skipping Channel logging, application does not appear to be SAP");
			return;
		}
		NewRelic.getAgent().getLogger().log(Level.FINE, "Call to initialize Communcation Channel Logging");

		if(ChannelMonitoringLogger.initialized) return;
