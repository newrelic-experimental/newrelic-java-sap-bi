...
public class IDocAdapterLoggingConfig {
	...
	public IDocAdapterLoggingConfig() {
		...
		if(logDirExists) {
			File logFile = new File(logDirectory, IDocAdapterLogger.LOGFILENAME);
			adapterLog = logFile.getAbsolutePath();
		} else {
			File logFile = new File(newRelicDir, IDocAdapterLogger.LOGFILENAME);
			adapterLog = logFile.getAbsolutePath();
		}
	}
