...
public class IDoc2LoggingConfig {
	...
	public IDoc2LoggingConfig() {
		...
		if(logDirExists) {
			File logFile = new File(logDirectory, IDoc2Logger.LOGFILENAME);
			adapterLog = logFile.getAbsolutePath();
		} else {
			File logFile = new File(newRelicDir, IDoc2Logger.LOGFILENAME);
			adapterLog = logFile.getAbsolutePath();
		}
	}
